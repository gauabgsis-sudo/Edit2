...
local function main()
	...
	Explorer.InitRightClick = function()
        ...
        context:Register("TELEPORT_TO",{Name = "Teleport To", IconMap = Explorer.MiscIcons, Icon = "TeleportTo", OnClick = function()
            ...
            -- ฟังก์ชัน teleport ทีละ stud
            local function stepTeleport(root, targetPos)
                ...
                while moved < distance do
                    -- เดินทีละ step
                    local newPos = startPos + direction * math.min(moved + step, distance)
                    root.CFrame = CFrame.new(newPos)
                    moved = moved + step
                    game:GetService("RunService").Heartbeat:Wait() -- รอ 1 เฟรม
                end
            end
            ...
            for i = 1,#sList do
                ...
                if isa(node.Obj,"BasePart") then
                    targetCFrame = node.Obj.CFrame + Settings.Explorer.TeleportToOffset
                elseif isa(node.Obj,"Model") then
                    if node.Obj.PrimaryPart then
                        targetCFrame = node.Obj.PrimaryPart.CFrame + Settings.Explorer.TeleportToOffset
                    else
                        local part = node.Obj:FindFirstChildWhichIsA("BasePart",true)
                        if part and nodes[part] then
                            targetCFrame = nodes[part].Obj.CFrame + Settings.Explorer.TeleportToOffset
                        end
                    end
                end

                if targetCFrame then
                    stepTeleport(hrp, targetCFrame.Position)
                    break
                end
            end
        end})
